	...
	class List_Post_Authors_Plus {
		...
		function onWidgetControl(  ) {
			if( isset( $_POST[ 'lpap_submit' ] ) ) {
				...
				$this->options[ 'show_gravatar' ] = $_POST[ 'lpap_show_gravatar' ];
				$this->options[ 'gravatar_size' ] = $_POST[ 'lpap_gravatar_size' ];
				$this->saveOptions();
			}
		...
		function listPostAuthors( $args ) {
			...
			foreach ( (array) $authors as $author ) {
				...
				if ( $posts == 0 ) {
					...
					if ($show_gravatar) {
					 $return .= get_avatar($author->email, $size = $gravatar_size);
					}
